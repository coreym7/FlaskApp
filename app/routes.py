    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if dict['Success']:
                ...
                return redirect(url_for('cmorris'))
            else:
...
def loginAPI():
    ...
    if username == 'cmorris' and password == '123':
        return jsonify(Success=True, uid=11)
    return jsonify(Success=False)

@app.route('/cmorris')
def cmorris():
    return render_template('cmorris.html')